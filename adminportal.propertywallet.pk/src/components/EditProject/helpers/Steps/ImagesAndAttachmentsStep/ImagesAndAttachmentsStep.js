...
const ImagesAndAttachmentsStep = ({ current, next, prev }) => {
  ...
  function onFinish() {
    ...
    if (
      createProjectStepTwo?.data === null &&
      getProjectDetailsStepTwo?.data?.data?.masterPlan === null &&
      getProjectDetailsStepTwo?.data?.data?.propertyWalletProjectPhoto
        ?.length === 0 &&
      getProjectDetailsStepTwo?.data?.data?.propertyWalletProjectDocument
        ?.length === 0
    ) {
      ...
      setAlreadySubmittedFiles((prev) => [...prev, ...tempNameArr])
      updateProjectStepTwoApi(dispatch, formData, onSuccess, params?.id)
    }
  }
  function onSuccess() {
    // deleteMasterFile(masterImages[0]?.name);
    next()
  }
  useEffect(() => {
    if (current === 1) {
      getProjectDetailsStepTwoApi(dispatch, params?.id)
    }
  }, [current])
  function deletePhotos(id, type) {
    const body = {
      propertyWalletProjectId: params?.id,
      type: type,
    }
    if (type !== 'MASTERPLAN') {
      body.photoId = id
        ...
        <Row gutter={24}>
          ...
          <Col lg={8} xs={24}>
            <div className="" style={{ alignItems: '' }}>
              {getProjectDetailsStepTwo?.data?.data?.masterPlan &&
              imagePreviews.length === 0 ? (
                <div className="relative w-fit">
                  <img
                    src={getProjectDetailsStepTwo?.data?.data?.masterPlan}
                    alt=""
