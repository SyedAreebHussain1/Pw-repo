...
const ProjectDetailsSteps = ({ current, next, handleStateChange, state }) => {
  ...
  const getProjectStepOne = useSelector((state) => state.getProjectStepOne)
  ...
  function onFinish(values) {
    ...
    updateProjectStepOneApi(dispatch, body, onSuccess, params.id)
  }
  function onSuccess() {
    next()
  }
  ...
  useEffect(() => {
    if (current === 0) {
      getProjectStepOneApi(dispatch, params.id)
    }
  }, [current])

  useEffect(() => {
    if (getProjectStepOne?.data) {
      form.setFieldsValue({
        projectName: getProjectStepOne?.data?.data?.projectName,
        NOC: getProjectStepOne?.data?.data?.NOC ? 'yes' : 'no',
        description: getProjectStepOne?.data?.data?.description,
        address: getProjectStepOne?.data?.data?.address,
        city: getProjectStepOne?.data?.data?.city,
        location: getProjectStepOne?.data?.data?.address,
      })
      if (getProjectStepOne?.data?.data?.colorCode1?.includes('#')) {
        setPickedColor({
          color: getProjectStepOne?.data?.data?.colorCode1,
          id: null,
        })
      }
      ...
      setMarkers([
        {
          lat: Number(getProjectStepOne?.data?.data?.latitude),
          lng: Number(getProjectStepOne?.data?.data?.longitude),
        },
      ])
    }
  }, [getProjectStepOne?.data])
