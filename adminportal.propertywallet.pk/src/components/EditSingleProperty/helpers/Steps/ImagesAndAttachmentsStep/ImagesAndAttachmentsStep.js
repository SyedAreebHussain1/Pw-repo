...
const ImagesAndAttachmentsStep = ({ current, next, prev }) => {
  ...
  function onFinish() {
    ...
    if (
      createStepTwoImages?.data === null &&
      getStepTwoImages?.data?.data?.propertyWalletProductPhoto?.length === 0 &&
      getStepTwoImages?.data?.data?.propertyWalletProductDocument?.length === 0
    ) {
      const tempNameArr = []
  ...
  useEffect(() => {
    if (current === 1) {
      filesCountProject.setState(0)
      filesCountLegal.setState(0)
      getStepTwoImagesApi(
        dispatch,
        updatePropertyWalletProductStepOne?.data?.data?.id
      )
  ...
  useEffect(() => {
    if (getStepTwoImages?.data) {
      filesCountProject.setState(
        projectImages.length +
          getStepTwoImages?.data?.data?.propertyWalletProductPhoto?.length
      )
      filesCountLegal.setState(
        legalPdf.length +
          getStepTwoImages?.data?.data?.propertyWalletProductDocument?.length
      )
    }
  }, [getStepTwoImages?.data, projectImages, legalPdf])
        ...
        <Row gutter={24}>
          ...
          <Col lg={8} xs={24}>
            <Upload
              ...
              supportedText={'Files Supported  JPG,JPEG,PDF'}
              // fileLimit={1}
              multiple
